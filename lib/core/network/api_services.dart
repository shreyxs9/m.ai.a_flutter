...
class ChatTransportService {
  const ChatTransportService(this.client);
  ...
  Future<SendMessageResult> sendMessage(
    ...
    if (status == 202 || data['pending'] == true) {
    ...
    return SendMessageResult(
      sessionId: data['session_id']?.toString(),
      maiaResponse: data['maia_response'] == null
          ? null
...
class SendMessageResult {
  ...
  final bool pending;
  final Message? maiaResponse;
}

class InferenceStatus {
  const InferenceStatus({required this.active, this.sessionId});

  factory InferenceStatus.fromJson(Object? json) {
  ...
  Map<String, dynamic> toJson() => {'active': active, 'session_id': sessionId};
}
